...
export const Caisse: React.FC<CaisseProps> = ({ boutiqueId, caissierId }) => {
  const [caisseView, setCaisseView] = useState<'caisse' | 'tickets'>('caisse');
  ...
  return (
    <div className="pb-40 pt-20 px-4 max-w-lg md:max-w-3xl lg:max-w-5xl mx-auto flex flex-col gap-6 animate-fade-in">
      ...
      {/* Caisse / Tickets tab switcher */}
      <div className="flex gap-2 justify-center mt-2">
        <button
          onClick={() => setCaisseView('caisse')}
          className={`flex items-center gap-2 px-6 h-11 rounded-2xl font-bold text-sm transition-all active:scale-95 ${
            caisseView === 'caisse'
        ...
        {filteredProducts.map((p) => {
          ...
          return (
            ...
            >
              ...
              <div className="h-40 flex items-center justify-center relative overflow-hidden bg-primary-container">
                {p.image_url ? (
      ...
      {/* Floating Island Cart Trigger — caisse view only */}
      {caisseView === 'caisse' && cart.length > 0 && (
      ...
      {/* Tickets tab — historique des ventes du jour */}
      {caisseView === 'tickets' && (
        <section className="text-left mt-2 flex flex-col gap-3">
          <div className="flex items-center justify-between">
            <h3 className="font-headline-sm text-primary font-bold">Ventes du Jour</h3>
            <span className="text-[10px] text-outline font-bold">{todaySales.length} ticket{todaySales.length !== 1 ? 's' : ''}</span>
          </div>
          {todaySales.length === 0 ? (
            <p className="text-sm text-outline text-center py-10 bg-white rounded-2xl border border-outline-variant">Aucune vente enregistrée aujourd'hui.</p>
          ) : (
            <div className="flex flex-col gap-2">
              {todaySales.map((sale) => (
                <Card
